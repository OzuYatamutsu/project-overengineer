...
After starting a local cluster, run the `deploy-kube-cluster.sh` script in the project root.

The initial value of secrets will be autogenerated and inserted as the secret `initial-redis-password`.

The control plane is deployed to the `default` namespace; the monitoring plane is deployed to the
`monitoring-plane` namespace.

You can modify the password at runtime by changing the values in vault (under the path `secret/data/...`)
...
Run any account-level setup tasks within `terraform/account.tf` (only to be run once per account):
```bash
cd terraform/
terraform init
terraform plan
...
The password is autogenerated and stored in the kubernetes secret `grafana-admin-password` (i.e. `kubectl get secret -n monitoring-plane grafana-admin-password -o jsonpath='{.data.password}' | base64 -d`).

#### Note regarding GitHub Actions
Build-time vars must be set for deployment to work.

| Variable                | Type     | Description                                                                                                    |
|-------------------------|----------|---------------------------------------------------------------------------------------------------------------|
| AWS_ACCOUNT_ID          | Variable | The account ID of the AWS account to be used to create resources.                                             |
| AWS_ACCESS_KEY_ID       | Secret   | The ID associated with the access key used to create resources.                                               |
...
| PAGERDUTY_TOKEN       | Secret   | An API key used for terraform to configure PagerDuty resources. An API key can be obtained after signing up for the free plan [here](https://www.pagerduty.com/sign-up/?type=free). |
...
**Started 2025-12-31, completed 2026-04-01.**
