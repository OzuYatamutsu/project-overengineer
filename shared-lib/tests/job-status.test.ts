...
test('can construct a new job update directly', () => {
  expect(new JobUpdate(TEST_JOB_ID, JobStatus.NEW, "", 0)).toBeTruthy()
})
test('can serialize a job update', () => {
  expect(new JobUpdate(TEST_JOB_ID, JobStatus.NEW, "", 0).serialize()).toBeTruthy()
})
test('can construct a new job update from json', async () => {
  expect(JobUpdate.fromJsonString(
    new JobUpdate(TEST_JOB_ID, JobStatus.NEW, "", 0).serialize()
  )).toBeTruthy()
})