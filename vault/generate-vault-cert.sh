...
openssl req -new -nodes -config vault/vault-openssl.conf -keyout vault/vault.key -out vault/vault.csr -reqexts req_ext
...
# Generate full chain
cat vault/vault.crt vault/vault-ca.crt > vault/vault-chain.pem
