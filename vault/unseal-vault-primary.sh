...
if ! vault status -address=https://${CONTAINER_NAME}.svc-vault.default.svc.cluster.local:8200 -tls-skip-verify | grep -q 'Initialized.*true'; then
  echo "Initializing Vault..."
  vault operator init -address=https://${CONTAINER_NAME}.svc-vault.default.svc.cluster.local:8200 -key-shares=1 -key-threshold=1 -format=json -tls-skip-verify > /tmp/vault-unseal-info.json
